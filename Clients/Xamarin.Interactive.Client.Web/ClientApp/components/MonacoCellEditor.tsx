...
export class MonacoCellEditor extends React.Component<
    MonacoCellEditorProps,
    MonacoCellEditorState
> {
    ...
    render() {
        return <div onClick={e => this.focus(e)} />;
    }
    ...
    buildEditor(elem: Element) {
        ...
        untypedEditor._view.eventDispatcher.addEventHandler(
            internalViewEventsHandler
        );
    ...
    async updateCodeCellStatus(buffer: string) {
        this.lastUpdateResponse = await this.props.blockProps.updateCodeCell(
            buffer
        );
        this.clearMarkedText();

        if (!this.lastUpdateResponse) return;
    ...
    onEditorMessage(message: EditorMessage) {
        ...
        if (message.target !== this.getKey()) return;

        if (message.type === EditorMessageType.setSelection) {
            const isBackwards = message.data.isBackwards;
            const isUpArrow =
                message.data.keyCode == EditorKeys.UP ||
                message.data.keyCode == EditorKeys.BACKSPACE;
    ...
    onKeyDown(e: monaco.IKeyboardEvent) {
        if (this.isCompletionWindowVisible()) return;
        if (e.keyCode == monaco.KeyCode.Enter) {
            if (this.onEnter(e)) e.preventDefault();
            e.stopPropagation();
            return;
        }
        let handled = false;
        if (
            (e.keyCode == monaco.KeyCode.UpArrow &&
                !this.isParameterHintsWindowVisible()) ||
            e.keyCode == monaco.KeyCode.LeftArrow
        )
            handled = this.handleEditorBoundaries(
                -1,
                e.keyCode == monaco.KeyCode.UpArrow
            );
        else if (
    ...
    isMonacoWidgetVisible(widgetClassName: string) {
        ...
        for (var i = 0; i < widgets.length; i++)
            if (widgets[i].classList.contains("visible")) return true;
        return false;
    }
    ...
    updateLayout() {
        ...
        this.editor!.layout({
            ...
            height: (this
                .editor as any)._view._context.viewLayout._linesLayout.getLinesTotalHeight()
        });
    }
    ...
    handleInternalViewEvents(events: any[]) {
        for (let i = 0, len = events.length; i < len; i++) {
            ...
            if (
                type == ViewEventType.ViewLinesInserted ||
                type == ViewEventType.ViewLinesDeleted
            )
                this.updateLayout();
        }
    }
    ...
    handleEditorBoundaries(dir: 1 | -1, useLineBoundary: boolean) {
        ...
        if (!useLineBoundary) isBeginning = isBeginning && pos.column === 1;
        let isEnd = pos.lineNumber === this.getEditorLines() && dir === 1;
        if (!useLineBoundary)
            isEnd =
                isEnd && this.getLineColumns(pos.lineNumber) + 1 === pos.column;
    ...
    moveSelection(firstLine: boolean, firstColumn: boolean) {
        ...
        let lineIndex = 1;
        if (!firstLine) lineIndex = this.getEditorLines();

        const targetColumn = firstColumn
            ? 1
            : (this.editor!.getModel() as any)._lines[lineIndex - 1].text
                  .length + 1;
